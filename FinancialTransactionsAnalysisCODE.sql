...
UPDATE 
  account_information$ SET[Account Status] = 'Unknown' 
WHERE 
...
UPDATE 
  customer_demographics$ --Filling missing Age values with an average
SET 
  [Age] = (
    SELECT 
      AVG([Age]) 
...
DELETE a 
FROM 
  customer_demographics$ a 
  JOIN (
    SELECT 
      MIN([Customer ID]) as id 
    FROM 
      customer_demographics$ 
    GROUP BY 
      [Customer ID] 
...
FROM 
  ...
  AND a.[Product Type] = b.[Product Type] 
...
FROM 
  ...
  JOIN [account_information$] ai ON td.[Account ID] = ai.[Account ID] 
  JOIN [customer_demographics$] cd ON ai.[Customer ID] = cd.[Customer ID] 
GROUP BY 
  cd.[Income Group], 
...
WITH TotalSpending AS (
  ...
  FROM 
    [transaction_details$] td 
    INNER JOIN [account_information$] ai ON td.[Account ID] = ai.[Account ID] 
    INNER JOIN [customer_demographics$] cd ON ai.[Customer ID] = cd.[Customer ID] 
  GROUP BY 
    cd.[Income Group]
),
...
FROM 
  GroupedSpending gs 
  INNER JOIN TotalSpending ts ON gs.[Income Group] = ts.[Income Group] 
ORDER BY 
  gs.[Income Group], 
  gs.[Merchant Category]
...
WITH TransactionCounts AS (
  ...
  FROM 
    ...
    JOIN [account_information$] ai ON cd.[Customer ID] = ai.[Customer ID] 
    JOIN [transaction_details$] td ON ai.[Account ID] = td.[Account ID] 
  GROUP BY 
    cd.[Income Group], 
    td.[Transaction Type]
), 
...
FROM 
  product_usage$ 
WHERE 
  [Product Type] IN ('Loan', 'Credit Card') 
GROUP BY 
  [Product Type];
